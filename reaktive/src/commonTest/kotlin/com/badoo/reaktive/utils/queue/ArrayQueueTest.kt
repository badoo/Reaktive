...
class ArrayQueueTest : QueueTests by QueueTestsImpl(ArrayQueue()) {
    ...
    @Test
    fun toList_returns_list_with_all_items_in_order_WHEN_offered_less_than_INITIAL_CAPACITY_items() {
        val originalList = listOf("a", "b", "c")
        offer(originalList)

        val list = queue.asList()

        assertEquals(originalList, list)
    }

    @Test
    fun toList_returns_list_with_all_items_in_order_WHEN_offered_INITIAL_CAPACITY_items() {
