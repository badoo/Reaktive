...
class FlatMapSingleTest
    ...
    fun subscribes_to_upstream() {
        flatMapUpstreamAndSubscribe { TestSingle() }
    ...
    fun produces_values_in_correct_order() {
        ...
        upstream.onComplete()
        inners[3].onSuccess(3)
        inners[4].onSuccess(4)

        observer.assertValues(0, null, 1, 3, 4)
    }

    @Test
    fun completes_WHEN_upstream_completed_without_values() {
        val observer = flatMapUpstreamAndSubscribe { TestSingle() }
    ...
    @Test
    fun completes_WHEN_upstream_produced_values_and_completed_and_all_inner_sources_succeeded() {
    ...
    @Test
    fun does_not_complete_WHEN_upstream_produced_values_and_completed_and_not_all_inner_sources_are_succeeded() {
        val inners = createInnerSources(3)
        val observer = flatMapUpstreamAndSubscribe(inners)

        upstream.onNext(0)
        inners[0].onSuccess(2)
        upstream.onNext(1, 2)
        upstream.onComplete()
        inners[2].onSuccess(2)
    ...
    private fun flatMapUpstreamAndSubscribe(innerSources: List<Single<Int?>>): TestObservableObserver<Int?> =
