...
class TestSchedulerWithManualProcessingTest {
    ...
    @Test
    fun task_not_called_WHEN_submitted_with_positive_delay_and_delay_reached() {
        scheduler.newExecutor().submit(delay = 1.milliseconds, task = task.ref)
        timer.advanceBy(1.milliseconds)
    ...
    @Test
    fun task_called_WHEN_submitted_with_positive_delay_and_delay_reached_and_processed() {
        scheduler.newExecutor().submit(delay = 1.milliseconds, task = task.ref)
        timer.advanceBy(1.milliseconds)
        scheduler.process()
    ...
    @Test
    fun task_not_called_WHEN_submitted_with_positive_delay_and_delay_over_reached() {
        scheduler.newExecutor().submit(delay = 1.milliseconds, task = task.ref)
    ...
    @Test
    fun task_not_called_WHEN_submitted_with_zero_delay_and_positive_period() {
    ...
    @Test
    fun task_called_WHEN_submitted_with_zero_delay_and_positive_period_and_processed() {
        scheduler.newExecutor().submit(delay = 0.milliseconds, period = 1.milliseconds, task = task.ref)
        scheduler.process()
    ...
    @Test
    fun task_not_called_WHEN_submitted_with_zero_delay_and_positive_period_and_period_reached() {
        scheduler.newExecutor().submit(delay = 0.milliseconds, period = 1.milliseconds, task = task.ref)
        scheduler.process()
        task.reset()
    ...
    @Test
    fun task_called_WHEN_submitted_with_zero_delay_and_positive_period_and_period_reached_and_processed() {
        scheduler.newExecutor().submit(delay = 0.milliseconds, period = 1.milliseconds, task = task.ref)
        scheduler.process()
        task.reset()
    ...
    @Test
    fun task_not_called_WHEN_submitted_with_delay_and_period_and_delay_reached() {
    ...
    @Test
    fun task_called_WHEN_submitted_with_delay_and_period_and_delay_reached_and_processed() {
        scheduler.newExecutor().submit(delay = 1.milliseconds, period = 1.milliseconds, task = task.ref)
    ...
    @Test
    fun task_not_called_WHEN_submitted_with_delay_and_period_and_period_reached() {
        scheduler.newExecutor().submit(delay = 1.milliseconds, period = 1.milliseconds, task = task.ref)
        timer.advanceBy(1.milliseconds)
        scheduler.process()
        task.reset()
    ...
    @Test
    fun task_called_WHEN_submitted_with_delay_and_period_and_period_reached_and_processed() {
        scheduler.newExecutor().submit(delay = 1.milliseconds, period = 1.milliseconds, task = task.ref)
        timer.advanceBy(1.milliseconds)
        scheduler.process()
        task.reset()
    ...
    @Test
    fun task_called_multiple_times_WHEN_submitted_with_zero_delay_and_period_and_timer_advanced_and_processed() {
    ...
    @Test
    fun task_called_multiple_times_WHEN_submitted_with_delay_and_period_and_timer_advanced_and_processed() {
