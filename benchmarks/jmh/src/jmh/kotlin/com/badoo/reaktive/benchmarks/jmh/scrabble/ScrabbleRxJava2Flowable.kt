...
open class ScrabbleRxJava2Flowable : ScrabbleBase() {
    ...
    override fun play(): List<Entry<Int, List<String>>> {
        ...
        val scoreOfALetter = { letter: Int -> Flowable.just(letterScores[letter - 'a'.toInt()]) }
        ...
        val toIntegerFlowable = { string: String -> Flowable.fromIterable(IterableSpliterator.of(string.chars().boxed().spliterator())) }
        ...
        val nBlanks = { word: String ->
            histoOfLetters(word)
                .flatMapPublisher { map -> Flowable.fromIterable(map.entries) }
                .flatMap(blank)
                .reduce { a, b -> a + b }
        }
        ...
        val first3 = { word: String -> Flowable.fromIterable(IterableSpliterator.of(word.chars().boxed().limit(3).spliterator())) }
        val last3 = { word: String -> Flowable.fromIterable(IterableSpliterator.of(word.chars().boxed().skip(3).spliterator())) }


        // Stream to be maxed
        val toBeMaxed = { word: String ->
            Flowable.just(first3(word), last3(word))
                .flatMap { it }
        }
